    ...
    return (
        <div className="flex justify-between bg-white dark:bg-slate-500 dark:shadow-slate-700 rounded-b-2xl shadow-2xl fixed top-0 w-full mb-4">
            ...
            {nav && <div className="md:flex text-lg font-semibold p-2 mr-6 hidden">
                ...
                {dark && <button onClick={() => setDark(false) ? setTheme('dark'): setTheme("light")} className="p-2 pr-6 text-xl pt-1.5"><CiLight/></button>}
                {!dark && <button onClick={() => setDark(true) ? setTheme('light'): setTheme("dark")} className="p-2 pr-6 text-xl pt-1.5"><MdDarkMode/></button>}
            </div>}
